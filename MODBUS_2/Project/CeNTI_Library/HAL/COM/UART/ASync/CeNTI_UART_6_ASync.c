...
#include "../../../../lib/Utils/CeNTI_Data_types.h"
    ...
    #include "usart.h"
#else
    #error UART_6_ASync does not support the CODE_CONFIGURATOR selected!
#endif
...
/* ************************************************************************************ */
	...
	#define UART6_ASYNC_TIMEOUT_TICK		(HAL_MAX_DELAY)
#endif
...
et_RET UART_6_ASync__Initialize(void)
{
        ...
        /* Get UART6 io descriptor. */
        uart_ret |= usart_async_get_io_descriptor(&USART_6, g_st_io_ptr);

        /* Enable UART6. */
        uart_ret |= usart_async_enable(&USART_6);
                           ...
                           size_t    tx_vec_size)
{
    ...
    CENTI_CHECK_PTR(tx_vec);
        ...
        /* Write function. */
        SERCOM6_USART_Write((uint8_t *)&(tx_vec[0]), tx_vec_size);
        ...
        {
            while(!(UART6_IsTxReady()));
            /* Write function. */
            UART6_Write(tx_vec[i]);
            while(!(UART6_IsTxDone()));
        }
        ...
        /* Write function. */
        hal_status = HAL_UART_Transmit(&huart6,\
                                       &(tx_vec[0]),\
                                       (uint16_t)tx_vec_size,\
                                       (uint32_t)UART6_ASYNC_TIMEOUT_TICK);
                          ...
                          size_t    rx_vec_size)
{
    ...
    if (rx_vec_size <= 0)
    {
        ...
        /* Read function. */
        SERCOM6_USART_Read((uint8_t *)&(rx_vec[0]), rx_vec_size);
        ...
        /* Read function. */
        hal_status = HAL_UART_Receive(&huart6,\
                                      &(rx_vec[0]),\
