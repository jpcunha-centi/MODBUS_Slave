...
/* ************************************************************************************ */
    ...
    {
        *level = (LED_GetValue() == 0) ? LEVEL_LOW : LEVEL_HIGH;
    }

    void GPIO_LED__ToggleLevel(void)
    {
        if (GPIO_LED__GetLevel() == LEVEL_LOW)
        {
            GPIO_LED__SetLevel(LEVEL_HIGH);
        }
        else
        {
            GPIO_LED__SetLevel(LEVEL_LOW);
    ...
    t_LEVEL GPIO_LED__GetLevel(void)
    {
        return (LED_GetValue() == 0) ? LEVEL_LOW : LEVEL_HIGH;
    }
