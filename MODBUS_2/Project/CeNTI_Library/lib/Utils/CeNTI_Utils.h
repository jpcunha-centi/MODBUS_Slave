...
#define CENTI_ASSERT(cond) {                                        \
        if (!(cond)) {                                              \
            CENTI_DEBUG__Warning("Assertion failed: %s", #cond);    \
            CENTI_DEBUG__Return(RET_ASSERT_FAILED);                 \
            return RET_ASSERT_FAILED;                               \
        }                                                           \
    }
...
/**
 * @brief Combines two uint16 values into one uint32 value.
 *
 * @param[in]   msb     Most significant bits
 * @param[in]   lsb     Least significant bits
 *
 * @return Combined uint32 value
 */
static inline uint32_t CENTI__2uint16_into_1uint32(uint16_t msb, uint16_t lsb)
    { return (((((uint32_t)msb)<<16)&0xFFFF0000) | ((uint32_t)(lsb&0x0000FFFF))); }


/**
 * @brief Converts four uint8 values into one uint32 value.
 *
 * @param[in]   msb     Most significant byte
 * @param[in]   mmsb    Middle most significant byte
 * @param[in]   mlsb    Middle least significant byte
 * @param[in]   lsb     Least significant byte
 *
 * @return the combined uint32 value
 */
static inline uint32_t CENTI__4uint8_into_1uint32(uint8_t msb, uint8_t mmsb, uint8_t mlsb, uint8_t lsb)
    { return (((((uint32_t)msb)<<24)&0xFF000000) | ((((uint32_t)mmsb)<<16)&0x00FF0000) | ((((uint32_t)mlsb)<<8)&0x0000FF00) | ((uint32_t)(lsb&0x000000FF))); }
