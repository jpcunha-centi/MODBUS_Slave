...
#include "../../../../lib/Utils/CeNTI_Data_types.h"
    ...
    #include "usart.h"
#else
    #error UART_4_ASync does not support the CODE_CONFIGURATOR selected!
#endif
...
/* ************************************************************************************ */
	...
	#define UART4_ASYNC_TIMEOUT_TICK		(HAL_MAX_DELAY)
#endif
...
/* * Public Functions                                                                 * */
/* ************************************************************************************ */
et_RET UART_4_ASync__Initialize(void)
{
    #if (defined(__CENTI_CODE_CONFIGURATOR_ATMEL_START__))
        ...
        /* Get UART4 io descriptor. */
        uart_ret |= usart_async_get_io_descriptor(&USART_4, g_st_io_ptr);

        /* Enable UART4. */
        uart_ret |= usart_async_enable(&USART_4);
...
et_RET UART_4_ASync__Write(uint8_t * tx_vec,\
                           size_t    tx_vec_size)
{
    ...
    CENTI_CHECK_PTR(tx_vec);
        ...
        /* Write function. */
        SERCOM4_USART_Write((uint8_t *)&(tx_vec[0]), tx_vec_size);
        ...
        /* Write function. */
        hal_status = HAL_UART_Transmit(&huart4,\
                                       &(tx_vec[0]),\
                                       (uint16_t)tx_vec_size,\
                                       (uint32_t)UART4_ASYNC_TIMEOUT_TICK);
...
et_RET UART_4_ASync__Read(uint8_t * rx_vec,\
                          size_t    rx_vec_size)
{
    /* Check if initialization was successfully complete. */
    CENTI_CHECK_INIT(g_initialized);
    ...
    if (rx_vec_size <= 0)
    {
        ...
        /* Read function. */
        SERCOM4_USART_Read((uint8_t *)&(rx_vec[0]), rx_vec_size);
        ...
        /* Read function. */
        hal_status = HAL_UART_Receive(&huart4,\
                                      &(rx_vec[0]),\
...
#endif /* (defined(__CENTI_UART_4_ASYNC__)) */
