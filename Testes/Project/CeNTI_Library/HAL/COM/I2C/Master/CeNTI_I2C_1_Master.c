...
#include "CeNTI_I2C_1_Master.h"
    ...
    #if (defined(SL_I2CSPM_CENTI_I2C_1_PRESENT))
        #include "sl_i2cspm_centi_i2c_1_config.h"
        #if (SL_I2CSPM_CENTI_I2C_1_PERIPHERAL_NO != 0)
            #error centi_i2c_1 instance is not using I2C1 peripheral!
        #else
            #if SL_I2CSPM_CENTI_I2C_1_SPEED_MODE == 0
                #define SL_I2CSPM_CENTI_I2C_1_HLR i2cClockHLRStandard
                #define SL_I2CSPM_CENTI_I2C_1_MAX_FREQ I2C_FREQ_STANDARD_MAX
            #elif SL_I2CSPM_CENTI_I2C_1_SPEED_MODE == 1
                #define SL_I2CSPM_CENTI_I2C_1_HLR i2cClockHLRAsymetric
                #define SL_I2CSPM_CENTI_I2C_1_MAX_FREQ I2C_FREQ_FAST_MAX
            #elif SL_I2CSPM_CENTI_I2C_1_SPEED_MODE == 2
                #define SL_I2CSPM_CENTI_I2C_1_HLR i2cClockHLRFast
                #define SL_I2CSPM_CENTI_I2C_1_MAX_FREQ I2C_FREQ_FASTPLUS_MAX
            #endif
        #endif
    #else
      #error Rename your I2C instance to centi_i2c_1!
    #endif
#elif (defined(__CENTI_CODE_CONFIGURATOR_NRFX__))
    #include <zephyr/drivers/i2c.h>
#else
    #error I2C_1_Master does not support the CODE_CONFIGURATOR selected!
#endif
...
/* ************************************************************************************ */
        ...
        #error I2C_1_Master needs to be configured with interrupts enabled!
    #endif
#endif
...
/* ************************************************************************************ */
	...
	#define I2C1_MASTER_TIMEOUT_TICK		(1000)

#elif (defined(__CENTI_CODE_CONFIGURATOR_NRFX__))

    #define I2C1_NODE DT_NODELABEL(centi_i2c_1)
...
/* IO structure. */
    ...
    static const struct i2c_dt_spec ST_DT_SPEC_I2C1 = I2C_DT_SPEC_GET(I2C1_NODE)
...
/* ************************************************************************************ */
     ...
     * 
     * @return i2c1_operations_t.
     */
    static i2c1_operations_t I2C_1_Master_ISR__Nack(void *ret_ptr);
     ...
     * 
     * @return i2c1_operation_t.
     */
    static i2c1_operations_t I2C_1_Master_Handler__WriteRead(void * ptr);
     ...
     * @return i2c1_operation_t.
     */
    static i2c1_operations_t I2C_1_Master_Handler__DoubleWrite(void * ptr);
...
/* * Public Functions                                                                 * */
/* ************************************************************************************ */
et_RET I2C_1_Master__Initialize(void)
{
    #if (defined(__CENTI_CODE_CONFIGURATOR_ATMEL_START__))
        ...
        /* Get I2C1 io descriptor. */
        i2c_ret |= i2c_m_sync_get_io_descriptor(&I2C_1, g_st_io_ptr);
        /* Enable I2C1. */
        i2c_ret |= i2c_m_sync_enable(&I2C_1);
        ...
        et_ret = (SERCOM_I2C_ERROR_NONE == SERCOM1_I2C_ErrorGet()) ? RET_OK : \
                                                                     RET_COM_I2C_FAIL;
        CENTI_CHECK_RET(et_ret);
        ...
        I2CSPM_Init_TypeDef init_centi_i2c_1 =
        {
          .port         = SL_I2CSPM_CENTI_I2C_1_PERIPHERAL,
          .sclPort      = SL_I2CSPM_CENTI_I2C_1_SCL_PORT,
          .sclPin       = SL_I2CSPM_CENTI_I2C_1_SCL_PIN,
          .sdaPort      = SL_I2CSPM_CENTI_I2C_1_SDA_PORT,
          .sdaPin       = SL_I2CSPM_CENTI_I2C_1_SDA_PIN,
          .i2cRefFreq   = SL_I2CSPM_CENTI_I2C_1_REFERENCE_CLOCK,
          .i2cMaxFreq   = SL_I2CSPM_CENTI_I2C_1_MAX_FREQ,
          .i2cClhr      = SL_I2CSPM_CENTI_I2C_1_HLR
        };

        I2CSPM_Init(&init_centi_i2c_1);
        ...
        i2c_dev_rdy = device_is_ready(ST_DT_SPEC_I2C1.bus);
...
et_RET I2C_1_Master__Write(uint8_t   i2c_address,\
                           uint8_t * tx_vec,\
                           size_t    tx_vec_size)
{
    ...
    CENTI_CHECK_INIT(g_initialized);
        ...
        /* Set slave address. */
        i2c_m_sync_set_slaveaddr(&I2C_1, i2c_address, I2C_M_SEVEN);
        /* Write function. */
        ...
        /* Write function. */
        SERCOM1_I2C_Write((uint16_t)i2c_address, &(tx_vec[0]), (uint32_t)tx_vec_size);
        ...
        /* Wait here until I2C 1 is open. */
        while (!I2C1_Open(i2c_address)) {}
        
        /* Set callback function for an address nack. */
        I2C1_SetAddressNackCallback(I2C_1_Master_ISR__Nack, &et_ret);
        /* Set callback function for a data nack. */
        I2C1_SetDataNackCallback(I2C_1_Master_ISR__Nack, &et_ret);
        ...
        I2C1_SetBuffer(&(tx_vec[0]), tx_vec_size);
        /* Write function. */
        I2C1_MasterWrite();
        
        /* Wait here until I2C 1 is not closed. */
        ...
        /* Write function. */
        I2C1_Write((uint16_t)i2c_address, &(tx_vec[0]), tx_vec_size);
        /* Check errors. */
        i2c_ret = I2C1_ErrorGet();
        et_ret  = (i2c_ret == I2C_ERROR_ADDR_NACK)      ? RET_COM_I2C_NACK  : \
                  (i2c_ret == I2C_ERROR_DATA_NACK)      ? RET_COM_I2C_NACK  : \
        ...
        transfer_ret = I2CSPM_Transfer(SL_I2CSPM_CENTI_I2C_1_PERIPHERAL, &transfer_seq);
        ...
        if (i2c_address == ST_DT_SPEC_I2C1.addr)
        {
            i2c_ret = i2c_write_dt(&ST_DT_SPEC_I2C1,\
...
et_RET I2C_1_Master__Read(uint8_t   i2c_address,\
                          uint8_t * rx_vec,\
                          size_t    rx_vec_size)
{
    /* Check if initialization was successfully complete. */
    CENTI_CHECK_INIT(g_initialized);
        ...
        /* Read function. */
        SERCOM1_I2C_Read((uint16_t)i2c_address, &(rx_vec[0]), (uint32_t)rx_vec_size);
        ...
        I2C1_SetDataCompleteCallback(NULL, NULL);
        /* Set buffer with data and size. */
        I2C1_SetBuffer(&(rx_vec[0]), rx_vec_size);
        /* Read function. */
        I2C1_MasterRead();
        ...
        /* Read function. */
        I2C1_Read((uint16_t)i2c_address, &(rx_vec[0]), rx_vec_size);
        /* Check errors. */
        i2c_ret = I2C1_ErrorGet();
        et_ret  = (i2c_ret == I2C_ERROR_ADDR_NACK)      ? RET_COM_I2C_NACK  : \
                  (i2c_ret == I2C_ERROR_DATA_NACK)      ? RET_COM_I2C_NACK  : \
        ...
        if (i2c_address == ST_DT_SPEC_I2C1.addr)
        {
            i2c_ret = i2c_read_dt(&ST_DT_SPEC_I2C1,\
...
et_RET I2C_1_Master__WriteRead(uint8_t   i2c_address,\
                               uint8_t * tx_vec,\
                               size_t    tx_vec_size,\
                               uint8_t * rx_vec,\
                               size_t    rx_vec_size)
{
    ...
    CENTI_CHECK_INIT(g_initialized);
        ...
        /* Write/Read function. */
        SERCOM1_I2C_WriteRead((uint16_t)i2c_address,\
                              &(tx_vec[0]), (uint32_t)tx_vec_size,\
                              &(rx_vec[0]), (uint32_t)rx_vec_size);
        ...
        I2C1_SetDataNackCallback(I2C_1_Master_ISR__Nack, &et_ret);
        /* Set callback function for when data transmission is complete. */
        I2C1_SetDataCompleteCallback(I2C_1_Master_Handler__WriteRead, &st_i2c_buffer);
        /* Set buffer with data and size. */
        I2C1_SetBuffer(&(tx_vec[0]), tx_vec_size);
        /* Write function. */
        I2C1_MasterWrite();
        ...
        /* Write/Read function. */
        I2C1_WriteRead((uint16_t)i2c_address,\
        ...
        if (i2c_address == ST_DT_SPEC_I2C1.addr)
        {
            i2c_ret = i2c_write_read_dt(&ST_DT_SPEC_I2C1,\
                                        &(tx_vec[0]), (uint32_t)tx_vec_size,\
                                        &(rx_vec[0]), (uint32_t)rx_vec_size);
            et_ret = (i2c_ret == 0) ? RET_OK : RET_COM_I2C_FAIL;
            CENTI_CHECK_RET(et_ret);
...
et_RET I2C_1_Master__DoubleWrite(uint8_t   i2c_address,\
                                 uint8_t * tx_vec_1,\
                                 ...
                                 size_t    tx_vec_2_size)
{
    ...
    CENTI_CHECK_INIT(g_initialized);
        ...
        I2C1_SetDataNackCallback(I2C_1_Master_ISR__Nack, &et_ret);
        /* Set callback function for when data transmission is complete. */
        I2C1_SetDataCompleteCallback(I2C_1_Master_Handler__DoubleWrite, &st_i2c_buffer);
        /* Set buffer with data and size. */
        I2C1_SetBuffer(&(tx_vec_1[0]), tx_vec_1_size);
        /* Write function. */
        I2C1_MasterWrite();
...
/* ************************************************************************************ */
    ...
    static i2c1_operations_t I2C_1_Master_Handler__WriteRead(void * ptr)
    {
        I2C1_SetBuffer(((st_I2C_buffer *)ptr)->data_vec,\
...
#endif /* (defined(__CENTI_I2C_1_MASTER__)) */
