...
/* * Includes                                                                         * */
/* ************************************************************************************ */
#include "CeNTI_I2C_5_Master.h"
#if (defined(__CENTI_CODE_CONFIGURATOR_ATMEL_START__))
    #include "driver_init.h"
#elif (defined(__CENTI_CODE_CONFIGURATOR_HARMONY__))
    #include "../../../../../../config/default/peripheral/sercom/i2c_master/plib_sercom5_i2c_master.h"
#elif (defined(__CENTI_CODE_CONFIGURATOR_MCC__))
    #include "../../../../../../mcc_generated_files/i2c5_master.h"
#elif (defined(__CENTI_CODE_CONFIGURATOR_MELODY__))
    #include "../../../../../../mcc_generated_files/i2c_host/i2c5.h"
#elif (defined(__CENTI_CODE_CONFIGURATOR_CUBEMX__))
    #include "i2c.h"
#elif (defined(__CENTI_CODE_CONFIGURATOR_SLCP__))
    #include "sl_i2cspm_instances.h"
    #if (defined(SL_I2CSPM_CENTI_I2C_5_PRESENT))
        #include "sl_i2cspm_centi_i2c_5_config.h"
        #if (SL_I2CSPM_CENTI_I2C_5_PERIPHERAL_NO != 0)
            #error centi_i2c_5 instance is not using I2C5 peripheral!
        #else
            #if SL_I2CSPM_CENTI_I2C_5_SPEED_MODE == 0
                #define SL_I2CSPM_CENTI_I2C_5_HLR i2cClockHLRStandard
                #define SL_I2CSPM_CENTI_I2C_5_MAX_FREQ I2C_FREQ_STANDARD_MAX
            #elif SL_I2CSPM_CENTI_I2C_5_SPEED_MODE == 1
                #define SL_I2CSPM_CENTI_I2C_5_HLR i2cClockHLRAsymetric
                #define SL_I2CSPM_CENTI_I2C_5_MAX_FREQ I2C_FREQ_FAST_MAX
            #elif SL_I2CSPM_CENTI_I2C_5_SPEED_MODE == 2
                #define SL_I2CSPM_CENTI_I2C_5_HLR i2cClockHLRFast
                #define SL_I2CSPM_CENTI_I2C_5_MAX_FREQ I2C_FREQ_FASTPLUS_MAX
            #endif
        #endif
    #else
      #error Rename your I2C instance to centi_i2c_5!
    #endif
#elif (defined(__CENTI_CODE_CONFIGURATOR_NRFX__))
    #include <zephyr/drivers/i2c.h>
#else
    #error I2C_5_Master does not support the CODE_CONFIGURATOR selected!
#endif
...
/* ************************************************************************************ */
        ...
        #error I2C_5_Master needs to be configured with interrupts enabled!
    #endif
#endif
...
/* ************************************************************************************ */
	...
	#define I2C5_MASTER_TIMEOUT_TICK		(1000)

#elif (defined(__CENTI_CODE_CONFIGURATOR_NRFX__))

    #define I2C5_NODE DT_NODELABEL(centi_i2c_5)
...
/* IO structure. */
    ...
    static const struct i2c_dt_spec ST_DT_SPEC_I2C5 = I2C_DT_SPEC_GET(I2C5_NODE)
...
/* ************************************************************************************ */
     ...
     * 
     * @return i2c5_operations_t.
     */
    static i2c5_operations_t I2C_5_Master_ISR__Nack(void *ret_ptr);
     ...
     * 
     * @return i2c5_operation_t.
     */
    static i2c5_operations_t I2C_5_Master_Handler__WriteRead(void * ptr);
     ...
     * @return i2c5_operation_t.
     */
    static i2c5_operations_t I2C_5_Master_Handler__DoubleWrite(void * ptr);
...
/* * Public Functions                                                                 * */
/* ************************************************************************************ */
et_RET I2C_5_Master__Initialize(void)
{
    #if (defined(__CENTI_CODE_CONFIGURATOR_ATMEL_START__))
        ...
        /* Get I2C5 io descriptor. */
        i2c_ret |= i2c_m_sync_get_io_descriptor(&I2C_5, g_st_io_ptr);
        /* Enable I2C5. */
        i2c_ret |= i2c_m_sync_enable(&I2C_5);
        ...
        et_ret = (SERCOM_I2C_ERROR_NONE == SERCOM5_I2C_ErrorGet()) ? RET_OK : \
                                                                     RET_COM_I2C_FAIL;
        CENTI_CHECK_RET(et_ret);
        ...
        I2CSPM_Init_TypeDef init_centi_i2c_5 =
        {
          .port         = SL_I2CSPM_CENTI_I2C_5_PERIPHERAL,
          .sclPort      = SL_I2CSPM_CENTI_I2C_5_SCL_PORT,
          .sclPin       = SL_I2CSPM_CENTI_I2C_5_SCL_PIN,
          .sdaPort      = SL_I2CSPM_CENTI_I2C_5_SDA_PORT,
          .sdaPin       = SL_I2CSPM_CENTI_I2C_5_SDA_PIN,
          .i2cRefFreq   = SL_I2CSPM_CENTI_I2C_5_REFERENCE_CLOCK,
          .i2cMaxFreq   = SL_I2CSPM_CENTI_I2C_5_MAX_FREQ,
          .i2cClhr      = SL_I2CSPM_CENTI_I2C_5_HLR
        };

        I2CSPM_Init(&init_centi_i2c_5);
        ...
        i2c_dev_rdy = device_is_ready(ST_DT_SPEC_I2C5.bus);
...
et_RET I2C_5_Master__Write(uint8_t   i2c_address,\
                           uint8_t * tx_vec,\
                           size_t    tx_vec_size)
{
    ...
    CENTI_CHECK_INIT(g_initialized);
        ...
        /* Set slave address. */
        i2c_m_sync_set_slaveaddr(&I2C_5, i2c_address, I2C_M_SEVEN);
        /* Write function. */
        ...
        /* Write function. */
        SERCOM5_I2C_Write((uint16_t)i2c_address, &(tx_vec[0]), (uint32_t)tx_vec_size);
        ...
        /* Wait here until I2C 5 is open. */
        while (!I2C5_Open(i2c_address)) {}
        
        /* Set callback function for an address nack. */
        I2C5_SetAddressNackCallback(I2C_5_Master_ISR__Nack, &et_ret);
        /* Set callback function for a data nack. */
        I2C5_SetDataNackCallback(I2C_5_Master_ISR__Nack, &et_ret);
        /* Set callback function for when data transmission is complete. */
        I2C5_SetDataCompleteCallback(NULL, NULL);
        /* Set buffer with data and size. */
        I2C5_SetBuffer(&(tx_vec[0]), tx_vec_size);
        /* Write function. */
        I2C5_MasterWrite();
        
        /* Wait here until I2C 5 is not closed. */
        while (I2C5_BUSY == I2C5_Close()) {}
        ...
        /* Write function. */
        I2C5_Write((uint16_t)i2c_address, &(tx_vec[0]), tx_vec_size);
        /* Check errors. */
        i2c_ret = I2C5_ErrorGet();
        et_ret  = (i2c_ret == I2C_ERROR_ADDR_NACK)      ? RET_COM_I2C_NACK  : \
                  (i2c_ret == I2C_ERROR_DATA_NACK)      ? RET_COM_I2C_NACK  : \
        ...
        /* Write function. */
        hal_status = HAL_I2C_Master_Transmit(&hi2c5,\
                                             (uint16_t)(i2c_address<<1),\
                                             &(tx_vec[0]),\
                                             tx_vec_size,\
                                             I2C5_MASTER_TIMEOUT_TICK);
        /* Check errors. */
        ...
        transfer_ret = I2CSPM_Transfer(SL_I2CSPM_CENTI_I2C_5_PERIPHERAL, &transfer_seq);
        ...
        if (i2c_address == ST_DT_SPEC_I2C5.addr)
        {
            i2c_ret = i2c_write_dt(&ST_DT_SPEC_I2C5,\
...
et_RET I2C_5_Master__Read(uint8_t   i2c_address,\
                          uint8_t * rx_vec,\
                          size_t    rx_vec_size)
{
    /* Check if initialization was successfully complete. */
    CENTI_CHECK_INIT(g_initialized);
        ...
        /* Read function. */
        SERCOM5_I2C_Read((uint16_t)i2c_address, &(rx_vec[0]), (uint32_t)rx_vec_size);
        ...
        I2C5_SetDataCompleteCallback(NULL, NULL);
        /* Set buffer with data and size. */
        I2C5_SetBuffer(&(rx_vec[0]), rx_vec_size);
        /* Read function. */
        I2C5_MasterRead();
        ...
        /* Read function. */
        I2C5_Read((uint16_t)i2c_address, &(rx_vec[0]), rx_vec_size);
        /* Check errors. */
        i2c_ret = I2C5_ErrorGet();
        et_ret  = (i2c_ret == I2C_ERROR_ADDR_NACK)      ? RET_COM_I2C_NACK  : \
                  (i2c_ret == I2C_ERROR_DATA_NACK)      ? RET_COM_I2C_NACK  : \
        ...
        /* Read function. */
        hal_status = HAL_I2C_Master_Receive(&hi2c5,\
                                            (uint16_t)(i2c_address<<1),\
                                            &(rx_vec[0]),\
                                            rx_vec_size,\
                                            I2C5_MASTER_TIMEOUT_TICK);
        ...
        if (i2c_address == ST_DT_SPEC_I2C5.addr)
        {
            i2c_ret = i2c_read_dt(&ST_DT_SPEC_I2C5,\
...
et_RET I2C_5_Master__WriteRead(uint8_t   i2c_address,\
                               uint8_t * tx_vec,\
                               size_t    tx_vec_size,\
                               uint8_t * rx_vec,\
                               size_t    rx_vec_size)
{
    ...
    CENTI_CHECK_INIT(g_initialized);
        ...
        /* Write/Read function. */
        SERCOM5_I2C_WriteRead((uint16_t)i2c_address,\
                              &(tx_vec[0]), (uint32_t)tx_vec_size,\
                              &(rx_vec[0]), (uint32_t)rx_vec_size);
        ...
        I2C5_SetDataNackCallback(I2C_5_Master_ISR__Nack, &et_ret);
        /* Set callback function for when data transmission is complete. */
        I2C5_SetDataCompleteCallback(I2C_5_Master_Handler__WriteRead, &st_i2c_buffer);
        /* Set buffer with data and size. */
        I2C5_SetBuffer(&(tx_vec[0]), tx_vec_size);
        /* Write function. */
        I2C5_MasterWrite();
        ...
        /* Write/Read function. */
        I2C5_WriteRead((uint16_t)i2c_address,\
        ...
        if (i2c_address == ST_DT_SPEC_I2C5.addr)
        {
            i2c_ret = i2c_write_read_dt(&ST_DT_SPEC_I2C5,\
                                        &(tx_vec[0]), (uint32_t)tx_vec_size,\
                                        &(rx_vec[0]), (uint32_t)rx_vec_size);
            et_ret = (i2c_ret == 0) ? RET_OK : RET_COM_I2C_FAIL;
            CENTI_CHECK_RET(et_ret);
...
et_RET I2C_5_Master__DoubleWrite(uint8_t   i2c_address,\
                                 uint8_t * tx_vec_1,\
                                 ...
                                 size_t    tx_vec_2_size)
{
    ...
    CENTI_CHECK_INIT(g_initialized);
        ...
        I2C5_SetDataNackCallback(I2C_5_Master_ISR__Nack, &et_ret);
        /* Set callback function for when data transmission is complete. */
        I2C5_SetDataCompleteCallback(I2C_5_Master_Handler__DoubleWrite, &st_i2c_buffer);
        /* Set buffer with data and size. */
        I2C5_SetBuffer(&(tx_vec_1[0]), tx_vec_1_size);
        /* Write function. */
        I2C5_MasterWrite();
...
/* ************************************************************************************ */
    ...
    static i2c5_operations_t I2C_5_Master_Handler__WriteRead(void * ptr)
    {
        I2C5_SetBuffer(((st_I2C_buffer *)ptr)->data_vec,\
                       ((st_I2C_buffer *)ptr)->data_vec_size);

        I2C5_SetDataCompleteCallback(NULL, NULL);

        return I2C5_RESTART_READ;
    }
    ...
    static i2c5_operations_t I2C_5_Master_Handler__DoubleWrite(void * ptr)
    {
        ...
        return I2C5_RESTART_WRITE;
    }

#endif


#endif /* (defined(__CENTI_I2C_5_MASTER__)) */
